...
public static class ValueExtensions {
	public static bool ValueExists(this LdapService ldap, string name) {
		SearchRequest   request  = new($"cn={name},ou=values,{ldap.DomainComponents}", LdapService.AnyFilter, SearchScope.Base, []);
	...
	public static string? GetValue(this LdapService ldap, string name, out string? error) {
		SearchRequest   request  = new($"cn={name},ou=values,{ldap.DomainComponents}", LdapService.AnyFilter, SearchScope.Base, "description");
