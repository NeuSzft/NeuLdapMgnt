...
namespace NeuLdapMgnt.Api.Endpoints
{
	public static class TeacherEndpoints
	{
		public static void MapTeacherEndpoints(this WebApplication app)
		{
			...
			{
				Teacher[] teachers = ldap.GetAllEntities<Teacher>().ToArray();
				return new RequestResult<Teacher>().SetValues(teachers).RenewToken(request).ToResult();
			}).WithOpenApi()
			  .WithTags("Teachers")
			  .RequireAuthorization()
			  .Produces<RequestResult<Teacher>>()
			...
			app.MapGet("/api/teachers/{id}", (LdapService ldap, HttpRequest request, long id) =>
			{
			...
			app.MapPut("/api/teachers/{id}", async (LdapService ldap, HttpRequest request, long id) =>
			{
				var result = await ModelValidator.ValidateRequest<Teacher>(request);
				if (result.IsFailure())
					return result.RenewToken(request).ToResult();
			...
			app.MapDelete("/api/teachers/{id}", (LdapService ldap, HttpRequest request, long id) =>
